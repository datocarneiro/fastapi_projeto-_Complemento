...
from app.models import Usuario, BaseUsuarioSchema



def insert_usuario(session_db, usuario: Usuario ) -> Usuario:
    usuario_db = Usuario(nome = usuario.nome, cpf = usuario.cpf, password = usuario.password)              
    session_db.add(usuario_db)
    session_db.commit()
    session_db.refresh(usuario_db)
    return usuario_db

def read_usuarios(session_db) -> BaseUsuarioSchema:
